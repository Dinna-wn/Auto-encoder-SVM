...
# importation du SVM lineaire
from sklearn.svm import LinearSVC
from sklearn.metrics import accuracy_score

from sklearn.preprocessing import StandardScaler

#importation de la validation croisée
...
mainDataPath = "../input/d/datasets/vache-elephant/vache_elephant/" #dans l'entrainement on a pas fait la decomression car on a travaillé dans Kaggle , et cet environement peut lire directemeent les fichiers zip sans les decompresses
...
training_ds_size = 1920  # total 2000 (1000 classe: 2 et 1000 classe: 7)
validation_ds_size = 480  # total 400 (200 classe: 2 et 200 classe: 7)


# Configuration des  images
image_scale = 112# la taille des images
...
fit_batch_size = 20  # le nombre d'images entrainées ensemble: un batch
...
def encoder(input):
    ...
    x = Conv2D(32, (4, 4), padding='same')(input) #couche de convolution de 32 filtres
    x = Activation('relu')(x)
    x = MaxPooling2D((2, 2), padding='same')(x)
    
    x = Conv2D(64, (4, 4), padding='same')(x) #couche de convolution de 64 filtres
    x = Activation('relu')(x)
    x = MaxPooling2D((2, 2), padding='same')(x)
    
    x = Conv2D(128, (4, 4), padding='same')(x) #couche de convolution de 128 filtres
    x = Activation('relu')(x)
...
def decoder(encoded):
    ...
    x = Conv2D(128, (4, 4), padding='same')(encoded)
    x = Activation('relu')(x)
...
model.summary()
base_learning_rate = 0.001 #pas d'apprentissage
...
model.compile(loss='mse', optimizer=tf.keras.optimizers.Adam(learning_rate=base_learning_rate), metrics=['mse'])
...
# Les images sont normalisées (leurs pixels divisées par 255)
validation_data_generator = ImageDataGenerator(rescale=1. / 255,validation_split=0.20)
...
training_generator = training_data_generator.flow_from_directory(
    ...
    subset='training', # nombre d'images total à entrainer
...
validation_generator = validation_data_generator.flow_from_directory(
    ...
    subset='validation', # nombre d'images total à valider
    class_mode ="input") # Comme nous somme entrain de reconstruire les images, alors
                        # la classe de chacune des pixels de sorite est le pixel d'entrée elle même(Input pixel)
